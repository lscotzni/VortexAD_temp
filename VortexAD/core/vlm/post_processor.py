...
def post_processor(num_nodes, mesh_dict, gamma, V_inf, alpha):
    ...
    induced_vel_force_points = csdl.Variable(shape=(num_nodes, num_total_panels), value=0.)
    for i in csdl.frange(num_nodes):
        induced_vel_force_points = induced_vel_force_points.set(csdl.slice[i,:], value=csdl.matvec(AIC_force_eval_pts[i,:,:], gamma[i,:]))
    ...
    for key in mesh_dict.keys():
        ...
        stop += num_panels
        output_dict[key]['force_pts_v_induced'] = induced_vel_force_points[:,start:stop].reshape((num_nodes, nc-1, ns-1))
        start += num_panels
