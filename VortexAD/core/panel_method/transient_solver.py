...
def transient_solver(mesh_dict, wake_mesh_dict, num_nodes, nt, num_tot_panels, dt, free_wake=False):
    ...
    mu_wake = csdl.Variable(shape=(num_nodes, nt, num_wake_panels - num_first_wake_panels), value=0.)
    ...
    for t in range(nt):
        ...
        # AIC_mu_total = AIC_mu_total.set(csdl.slice[:,t,:,:], value=AIC_mu[:,t,:,:]*1.)
        ...
        for i in range(num_surfaces):
            ...
            mu_wake_grid = mu_wake[:,t,start_i_w:stop_i_w].reshape((num_nodes, nc_w-2, ns_w-1))
            ...
            mu_wake = mu_wake.set(csdl.slice[:,t+1,start_i_w:stop_i_w], value=mu_wake_grid_next.reshape((num_nodes, (nc_w-2)*(ns_w-1))))
            ...
            if free_wake:
                total_vel = wake_velocity_timestep + induced_vel
            else:
