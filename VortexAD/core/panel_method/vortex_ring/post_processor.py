...
def post_processor(mesh_dict, gamma, num_nodes, nt, dt):
    ...
    for i in range(len(surface_names)):
        ...
        gamma_grid = gamma[:,:,start:stop].reshape((num_nodes, nt, nc-1, ns-1))
        ...
        if True:
            ...
            # ql, qm = least_squares_velocity_old(gamma_grid, delta_coll_point)
            ql, qm = least_squares_velocity(gamma_grid, delta_coll_point)
            # endregion
        if False:
            ...
            ql, qm = perturbation_velocity_FD(gamma_grid, dl, dm)
            # endregion
        ...
        if False:
            ...
            ql, qm = perturbation_velocity_FD_K_P(gamma_grid, panel_center_dl, panel_center_dm)
            # endregion
        ...
        dgamma_dt = csdl.Variable(shape=Q_inf_norm.shape, value=0)
        if nt > 2:
