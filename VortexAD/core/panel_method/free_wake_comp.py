...
from VortexAD.core.panel_method.source_functions import compute_source_influence
from VortexAD.core.panel_method.doublet_functions import compute_doublet_influence
from VortexAD.core.panel_method.vortex_line_functions import compute_vortex_line_ind_vel
...
def free_wake_comp(num_nodes, t, mesh_dict, mu, sigma, wake_mesh_dict, mu_wake):
    ...
    for i in range(len(surface_names)):
        ...
        for j in range(len(surface_names)):
            ...
            ind_vel_s_12 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,0,:],panel_corners_s_j_exp_vec[:,:,1,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:])
            ind_vel_s_23 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,1,:],panel_corners_s_j_exp_vec[:,:,2,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:])
            ind_vel_s_34 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,2,:],panel_corners_s_j_exp_vec[:,:,3,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:])
            ind_vel_s_41 = compute_vortex_line_ind_vel(panel_corners_s_j_exp_vec[:,:,3,:],panel_corners_s_j_exp_vec[:,:,0,:],p_eval=eval_pt_w_i_exp_vec[:,:,0,:])
            ...
            ind_vel_w_12 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,0,:],panel_corners_w_j_exp_vec[:,:,1,:],p_eval=eval_pt_w_i_exp_vec[:,:,:])
            ind_vel_w_23 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,1,:],panel_corners_w_j_exp_vec[:,:,2,:],p_eval=eval_pt_w_i_exp_vec[:,:,:])
            ind_vel_w_34 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,2,:],panel_corners_w_j_exp_vec[:,:,3,:],p_eval=eval_pt_w_i_exp_vec[:,:,:])
            ind_vel_w_41 = compute_vortex_line_ind_vel(panel_corners_w_j_exp_vec[:,:,3,:],panel_corners_w_j_exp_vec[:,:,0,:],p_eval=eval_pt_w_i_exp_vec[:,:,:])
    ...
    for nn in csdl.frange(num_nodes):
        for dir in csdl.frange(3):
            # sigma_surf_induced_vel = csdl.matvec(AIC_sigma[nn,:,:,dir], sigma[])
            mu_surf_induced_vel = csdl.matvec(AIC_mu_surf[nn,:,:,dir], mu[nn,t,:])
            mu_wake_induced_vel = csdl.matvec(AIC_mu_wake[nn,:,:,dir], mu_wake[nn,t,:])
            induced_vel = induced_vel.set(csdl.slice[nn,:,dir], value=mu_surf_induced_vel+mu_wake_induced_vel)
